...
import ink.whi.project.common.utils.DateUtil;
...
public class SqlStateInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            ...
            // 这个方法的总耗时
//            log.info("\n\n ============= \nsql ----> {}\ndate ----> {}\ncost ----> {}\n ============= \n", sql, DateUtil.time2date(System.currentTimeMillis()), cost);
        }

        // 将日志写入sql.log文件，字符集utf-8
        String sqlLog = String.format("%s %s cost: %s\n", DateUtil.time2date(System.currentTimeMillis()), sql, cost);

        try {
            String logPath = "/Users/qing/MyDocuments/code/ideaProject/zhishu-match/src/main/resources/data/sql.log";
            File file = new File(logPath);
            if (!file.exists()) {
                FileUtil.touch(file);
            }

            FileUtil.appendString(sqlLog + "\n", logPath, StandardCharsets.UTF_8);
        } catch (Exception e) {
            log.error("write sql log error: {}", e.getMessage());
        }
